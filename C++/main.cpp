...
#define synchronized(MUTEX) if (bool _stop_ = false) {} else for (boost::scoped_guard<boost::mutex> _lock_(MUTEX); !_stop_; _stop_ = true)
...
std::vector<int>
asyncAttr(Graph* G, std::vector<int>* tmpMap, int inode, std::vector<bool>* removed, int i){
    std::vector<int> A;
    Node* node = &G->get(A[inode]);
...
bool
addAllEx(std::vector<int>& A, std::vector<int>& B, std::vector<int>& check){
    bool res = false;
    for (int i : B) {
        if(check[i]) continue;
        check[i] = 1;
...
Attr(Graph& G, std::vector<bool>& removed, std::vector<int>& A, int i) {
    ...
    while (index < A.size()) {
        while (index < A.size()) {
            tasks.push_back(std::async(asyncAttr, &G, &tmpMap, A[index], &removed, i));
            index += 1;
        }
